...
def get_barcode_no_data():
	pass
	# itemwise_barcode ={}
	# barcode = frappe.db.sql("""select name, barcode from `tabItem`""",as_dict=1)

	
def batch_no_pro_exp():
	query =frappe.db.sql("""SELECT `item`,`name`,`batch_id` as batch_id,  date(production_date) as production_date,
	 date(expiry_date) exp  from `tabBatch` where  DATEDIFF(expiry_date, date(now())) >= 2""",as_dict=1)
	batches ={}
	d ={}      
	for i in query:                    
	    d[i['item']] = min([row['exp'] for row in query if row['item'] == i['item']])
	for i in query:
	    if i['exp'] in d.values():
	    	batches[i['item']] =i
	return batches
