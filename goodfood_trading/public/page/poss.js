...
erpnext.pos.PointOfSale = erpnext.taxes_and_totals.extend({
	...
	get_data_from_server: function(callback){
		...
		frappe.call({
			method: "goodfood_trading.customization.poss.get_pos_data",
			freeze: true,
...
>>>>>>> be7b8e7d97b865d7a4affc3a34cb3a8495a1328a
	...
	update_qty: function(item_code, qty, item_idx, batch_no) {
		...
		this.update_qty_rate_against_item_code(item_code, "qty", qty, item_idx, batch_no);
	},
	...
	update_qty_rate_against_item_code: function(item_code, field, value, item_idx, batch_no){
		var me = this;
		if(value < 0){
	...
	remove_zero_qty_item: function(){
		...
		this.items = []
		idx = 0
		// console.log(JSON.stringify([me.remove_item, d.idx]))
		$.each(this.frm.doc["items"] || [], function(i, d) {
